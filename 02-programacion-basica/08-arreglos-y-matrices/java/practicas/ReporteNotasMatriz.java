public class ReporteNotasMatriz {
	...
	public static void main(String[] args) {
		...
		String[] nombres = { "USUARIO CHILETE", "APRENDIZ DEVPATH", "CLIENTE CHILETE", "ESTUDIANTE DEMO",
				"INTEGRANTE PRACTICA" };
		int[] edad = { 25, 24, 25, 19, 22 };
		int[] grado = { 5, 5, 5, 5, 4 };

		int[][] notas = { { 20, 17, 18, 16 }, { 16, 18, 15, 12 }, { 20, 20, 14, 15 }, { 20, 19, 15, 17 },
				{ 20, 18, 12, 13 } };
		...
		// El bucle recorre los datos siguiendo una cantidad o rango definido.
		...
		reporte.append(String.format("%s %s %s %s %s %s %s %s%n", "NOMBRE", "EDAD", "GRADO", "PC1", "PC2", "PC3",
				"EF", "PROMEDIO"));
		reporte.append(String.format("########################################################################################%n"));
		for (int i = 0; i < nombres.length; i++) {
			reporte.append(String.format("%-35s %-5d %-6d %-4d %-4d %-4d %-4d %-9.2f\n", nombres[i], edad[i],
					grado[i], notas[i][0], notas[i][1], notas[i][2], notas[i][3], promedio[i]));
		}
		reporte.append(String.format("########################################################################################%n"));
